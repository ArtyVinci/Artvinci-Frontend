...
const ArtworkCard = ({ artwork, onLike }) => {
  ...
  const imageUrl =
    artwork.primary_image || artwork.images?.[0]?.url || artwork.image || null;

  // Get artist name
  const artistName =
    artwork.artist?.username || artwork.artist_name || "Unknown Artist";
            ...
            <div className="flex flex-col">
              ...
              {artwork.likes_count > 0 && (
                <span className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  {artwork.likes_count} like
                  {artwork.likes_count !== 1 ? "s" : ""}
                </span>
              )}
            </div>
